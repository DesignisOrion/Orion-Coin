...
class SlitherClient extends plugin_1.PluginClient {
    ...
    mapNpmDepsDir(list) {
        ...
        const npmDepsExists = fs_1.existsSync(remixNpmDepsPath);
        const nodeModulesExists = fs_1.existsSync(localNpmDepsPath);
        ...
        for (const e of list) {
            ...
            if (nodeModulesExists && fs_1.readdirSync(localNpmDepsPath).includes(packageName)) {
                isLocalDep = true;
                remapString += `${packageName}=./node_modules/${packageName} `;
            }
            else if (npmDepsExists && fs_1.readdirSync(remixNpmDepsPath).includes(packageName)) {
                isRemixDep = true;
    ...
    analyse(filePath, compilerConfig) {
        return new Promise((resolve, reject) => {
            ...
            const fileContent = fs_1.readFileSync(utils.absolutePath(filePath, this.currentSharedFolder), 'utf8');
            ...
            const outputFile = 'remix-slitherReport_' + Math.floor(Date.now() / 1000) + '.json';
            ...
            child.on('close', () => {
                ...
                if (fs_1.existsSync(outputFileAbsPath)) {
                    let report = fs_1.readFileSync(outputFileAbsPath, 'utf8');
                    report = JSON.parse(report);
                    fs_1.unlink(outputFileAbsPath, (err) => {
                        if (err)
                            console.log(err);
                    });
                    if (report['success']) {
